...
def send_activation_email(user):
    ...
    if form.is_valid():
        ...
        form.save(
            ...
            from_email='Team Beato Guitar<hello@beatoguitar.com>',
            html_email_template_name='registration/account_activation_email.html'
        )

def webhook_is_verified(data, request_hmac):
    digest = hmac.new(settings.SHOPIFY_WEBHOOK_SECRET.encode('utf-8'),
...
def has_purchased_course(request_body):
    return any(pid in request_body for pid in settings.SHOPIFY_PRODUCT_IDS)
